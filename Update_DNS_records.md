...
2. DNS-записи при вводе машины в домен
	1. Присоединение к домену и настройка прав на DNS-записи
...
#### Присоединение к домену и настройка прав на DNS-записи

> В этом разделе рассказывается о процессе ввода машины в домен. Эти действия не требуют ручного выполнения.

При вводе машины в домен Active Directory вызывается утилита system-auth, который в свою очередь использует команду  **net ads join** ,  которая выполняет присоединение к домену с использованием Kerberos-аутентификации и отключением автоматического обновления DNS. После успешного присоединения машина регистрирует свою DNS-запись с помощью команды **net ads dns register** также через Kerberos-аутентификацию.

Во время регистрации машины в доменном DNS, system-auth использует билет Администратора и, как следствие, владельцем записи становится он, а не машина, что не дает машине обновить свою DNS-запись.

Имеются ввиду записи вида:

	DC=host1,DC=domain.alt,CN=MicrosoftDNS,DC=DomainDnsZones,DC=domain,DC=alt.

> Исправлено 8 июня 2024 года в пакете alterator-auth версии 0.44.10-alt1.

Данное исправление работает только для машин, которые не были ранее введены в домен с текущим именем (т.е. машинная учётная запись создаётся впервые). Если машина уже была добавлена в домен, но для неё используется новое имя, то будет создана новая учётная запись, и проблема с правами на обновление DNS-записей будет решена. Однако, если машина повторно вводится в домен с тем же именем, это не устранит проблему.
...
+ Воспользоваться c Windows машины RSAT оснасткой DNS и задать необходимые права вручную.
...
Версии пакетов, необходимых для корректного функционирования обновления DNS-записей:
