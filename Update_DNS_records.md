...
PTR запись будет автоматически создана, если на клиенте включены необходимые настройки для обновления PTR DNS-записей. В обратном случае запись нужно будет создать вручную:
...
*Описание:*  false (разрешить nsupdate выбрать протокол)
...
### Samba Winbind
---
Winbind – это сервис , работающий на клиентах Samba и обеспечивающий взаимодействие между PAM и NSS, работающими на машинах под управлением операционных систем на базе ядра Linux, с одной стороны, и Active Directory, работающей на контроллере домена, с другой. В частности, Winbind использует Kerberos для проверки подлинности с помощью Active Directory и LDAP для получения информации о пользователях и группах.
...
**Для машин под управлением ОС «Альт» предусмотрена установка пакетом:**
	...
	# apt-get install winbind-dnsupdate
...
Измененный файл при вызове команды # systemctl edit winbind-dnsupdate.timer выглядит следующим образом:
